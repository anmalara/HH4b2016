...
{
  TLegend *leg=new TLegend(0.7, 0.1, 0.9, 0.85);
  double wherepeak=(h2->GetMean())/(h2->GetXaxis()->GetXmax());
...
RooPlot* fitSignal(TH1F *h, std::string mass, int color, TLegend *leg, Params &params, double bias, bool kinFit=false)
{
  RooMsgService::instance().setGlobalKillBelow(RooFit::WARNING) ; gErrorIgnoreLevel = kWarning;
  ...
  RooRealVar signal_p0_1("signal_p0_1", "signal_p0_1", sg_p0->getVal());
  ...
  w->SaveAs((dest_dir+"/"+"SignalFits_LMR"+reg+"/w_signal_"+mass+".root").c_str());
  }
  return plot;
}
...
{ 
  ...
  RooWorkspace *w=new RooWorkspace("HbbHbb");
  w->import(signal_fixed);
  w->SaveAs((dest_dir+"/"+"SignalFits_LMR"+reg+"/w_signal_Gaussian_"+mass+".root").c_str());
  }
  return plot;
}
...
int Display_SignalFits_LMR_bias(std::string dir_preselection="PreselectedWithRegressionDeepCSV", std::string _reg = "reg",
			std::string dir_selection="LMRSelection_chi2", std::string _dest_dir=".",
			std::string file_histograms="Histograms_GluGluToBulkGravitonToHHTo4B_M-",
			int _mass=400,
			bool focus=false, int range = 1)
{
  ...
  if(range==1){
	switch(_mass){
<<<<<<< HEAD
		case 260: bias=0.1464; break;
		case 270: bias=0.1360; break;
		case 300: bias=-0.2192; break;
=======
		case 260: bias=-0.041; break;
		case 270: bias=-0.076; break;
		case 300: bias=-0.056; break;
>>>>>>> Daniel/master
		default: break;
	}
  }
	...
	switch(_mass){
<<<<<<< HEAD
		case 300: bias=0.0154; break;
		case 350: bias=-0.0224; break;
		case 400: bias=0.0182; break;
		case 450: bias=0.0130; break;
		case 500: bias=0.0029; break;
		case 550: bias=-0.0092; break;
		case 600: bias=-0.0035; break;
=======
		case 300: bias=0.037; break;
		case 350: bias=0.006; break;
		case 400: bias=-0.001; break;
		case 450: bias=-0.003; break;
		case 500: bias=-0.003; break;
		case 550: bias=-0.004; break;
		case 600: bias=0.001; break;
>>>>>>> Daniel/master
		default: break;
	}
  }
  ...
  v_zero.push_back(0);
  string width = "";
  if(masses.at(i) =="1000" || masses.at(i) =="1200") file_histograms = "Histograms_LMR_BulkGravTohhTohbbhbb_narrow_M-";
  else width = "_narrow";
  ...
  RooPlot *plot=fitSignal(h_mX_SR, masses.at(i), kBlack, leg, par, bias);
  ...
  if (!focus)
  {
    ...
    plot_JECp1=fitSignal(h_mX_SR_JECp1, masses.at(i), kRed, leg, par_JECp1, bias);
    plot_JECm1=fitSignal(h_mX_SR_JECm1, masses.at(i), kRed+2, leg, par_JECm1, bias);
  }
  ...
  RooPlot *plot_KinFit=fitSignal(h_mX_SR_KinFit, masses.at(i), kBlack, leg, par_KinFit, bias, true);
  ...
  if (focus)
  {
    leg->AddEntry(h_mX_SR_KinFit, "Gaus+Gaus fit", "L");
  ...
  if (!focus) {
	...
	plot_JECp1_KinFit=fitSignal(h_mX_SR_JECp1_KinFit, masses.at(i), kRed, leg, par_JECp1_KinFit, bias, true);
	plot_JECm1_KinFit=fitSignal(h_mX_SR_JECm1_KinFit, masses.at(i), kRed+2, leg, par_JECm1_KinFit, bias, true);

	leg->AddEntry(h_mX_SR_JERp1_KinFit, "JER +1 #sigma");
	leg->AddEntry(h_mX_SR_JERm1_KinFit, "JER -1 #sigma"); 
	plot_JERp1_KinFit=fitSignal(h_mX_SR_JERp1_KinFit, masses.at(i), kAzure+1, leg, par_JERp1_KinFit, bias, true);
	plot_JERm1_KinFit=fitSignal(h_mX_SR_JERm1_KinFit, masses.at(i), kBlue+1, leg, par_JERm1_KinFit, bias, true);

	leg->AddEntry(h_mX_SR_bTagDown_KinFit, "bTag +1 #sigma");
	leg->AddEntry(h_mX_SR_bTagUp_KinFit, "bTag -1 #sigma");

	plot_bTagDown_KinFit=fitSignal(h_mX_SR_bTagDown_KinFit, masses.at(i), kGreen+1, leg, par_bTagDown_KinFit, bias, true);
	plot_bTagUp_KinFit=fitSignal(h_mX_SR_bTagUp_KinFit, masses.at(i), kGreen+3, leg, par_bTagUp_KinFit, bias, true);
  }
